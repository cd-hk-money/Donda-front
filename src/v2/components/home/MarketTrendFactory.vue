      ...
      <div>
        <v-card-title class="d-flex align-center">
          <div class="market-title text-h6 font-weight-bold d-flex align-center">
            ...
            <v-icon :color="computedMarket.trendIconColor" small class="ml-1">{{
              computedMarket.trendIcon
            }}</v-icon>
          </div>
          <div class="mt-1">
            <span class="text-h5">
        ...
        <v-card-subtitle>
          ...
          <div>
            <v-chip class="mt-1">
              ...
              <span :class="['mr-1 ml-1 font-weight-bold', chipContnet.textColor]">
								{{ chipContnet.trend }}
							</span>중입니다.              
            </v-chip>
          </div>
...
export default class MarketTrendFactory extends mixins(StoreMixin, DiviceMixin) {
  ...
  get flag() {
    return this.contry === "korea"
      ? "@/assets/koreaflag.png"
      : "@/assets/usaflag.png";
  }
  ...
  get Picked() {
    ...
    return [
      new Date(last.setDate(last.getDate() - REQUEST_DATE[this.selectionChipGroup]))
        .toISOString()
        .substr(0, 10),
      this.dateLastDay,
    ];
